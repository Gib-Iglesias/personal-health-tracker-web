    ...
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-text-muted text-sm mt-1">{activeClients.length} activos, {mockClients.length - activeClients.length} en pausa</p>
        </div>
        ...
        {mockClients.map((c) => (
          <div key={c.id} onClick={() => setSelectedClient(selectedClient === c.id ? null : c.id)}
            ...
            <div className="flex items-center gap-4">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-[15px] font-bold text-white shrink-0 ${c.status === "active" ? "gradient-primary" : "bg-border"}`}>{c.avatar}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[15px] font-semibold text-foreground">{c.name}</span>
                  {c.streak > 0 && <span className="flex items-center gap-0.5 text-[11px] text-warning"><Flame size={12} /> {c.streak} días</span>}
                </div>
            ...
            {selectedClient === c.id && (
              <div className="mt-4 pt-4 border-t border-border grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
                ...
                <div><div className="flex items-center gap-1 text-xs text-text-muted"><Phone size={11} /> Teléfono</div><div className="text-sm text-foreground mt-0.5">{c.phone}</div></div>
                <div><div className="flex items-center gap-1 text-xs text-text-muted"><CalendarCheck size={11} /> Miembro desde</div><div className="text-sm text-foreground mt-0.5">{c.joinedDate}</div></div>
                <div><div className="flex items-center gap-1 text-xs text-text-muted"><Activity size={11} /> Grasa corporal</div><div className="text-sm text-foreground mt-0.5">{c.bodyFat ?? "—"}%</div></div>
                {c.notes && <div className="col-span-full"><div className="flex items-center gap-1 text-xs text-text-muted"><AlertTriangle size={11} /> Notas</div><div className="text-sm text-warning/80 mt-0.5">{c.notes}</div></div>}
              </div>
            )}
          </div>
        ))}
